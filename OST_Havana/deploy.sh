...
# configure the super-user admin token
# FIXME! Failed with this sed
sudo sed -i "s/^# admin_token.*/admin_token = ADMIN" \
	/etc/keystone/keystone.conf

# Keystone supports PKI infrastructure to cryptographically sign the tokens
# To disable this feature for now to use non-signed tokens
# FIXME! Failed with this sed
